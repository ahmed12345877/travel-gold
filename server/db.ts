...
/**
 * دالة الاتصال بـ Supabase Postgres مع الحفاظ على بنية الكود الأصلي
 */
  ...
  if (!_db && process.env.DATABASE_URL) {
    try {
      const pool = new Pool({
        ...
        ssl: { rejectUnauthorized: false } // لتجاوز مشاكل شهادات الأمان السحابية
      });
      _db = drizzle(pool);
      console.log("[Database] Connected successfully to Supabase PostgreSQL.");
    } catch (error) {
      console.error("[Database] Connection to Supabase failed:", error);
    }
  }
  return _db;
}
export async function createBooking(data: any) {
  ...
  const result = await dbClient.insert(bookings).values(data).returning();
  return result[0] || null;
}

export async function getBookingById(id: number) {
  const dbClient = await getDb();
  const result = await dbClient.select().from(bookings).where(eq(bookings.id, id)).limit(1);
  return result[0] || null;
}
