...
export default function BookingsAdmin() {
  ...
  const { data: bookingsData, isLoading } = trpc.bookings.listAll.useQuery({
    limit: 50,
    offset: 0,
  });
  const bookings = bookingsData || [];

  const filtered = (Array.isArray(bookings) ? bookings : []).filter(
    (b: any) => {
      const matchSearch =
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span
        className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border ${styles[status] || styles.pending}`}
      >
              ...
              {filtered.map((booking: any) => (
                ...
                >
                  ...
                  <td className="p-3 text-xs text-white/40">
                    {booking.travelDate
                      ? new Date(booking.travelDate).toLocaleDateString("ar-EG")
                      : "—"}
                  </td>
                  <td className="p-3">
                    <div className="flex items-center gap-1">
      ...
      {selectedBooking && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[var(--theme-surface)] border border-white/10 rounded-xl w-full max-w-lg p-6 space-y-4">
            ...
            <Button
              onClick={() => setSelectedBooking(null)}
              className="w-full bg-[var(--theme-primary)] text-black hover:bg-[var(--theme-primary-light)] font-semibold"
            >
              إغلاق
            </Button>
          </div>
