...
export default function NavbarAdmin() {
  ...
  const saveToDb = useCallback(async (items: MenuItem[]) => {
    setSaving(true);
    try {
  ...
  if (isLoading) return <div className="flex items-center justify-center h-64"><Loader2 className="animate-spin text-[var(--theme-primary)]" size={32} /><span className="mr-3 text-white/60">جاري تحميل بيانات القائمة...</span></div>;
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">{total} عنصر ({vis} مرئي)</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={() => saveToDb(menuItems)} disabled={saving || !hasChanges} className={hasChanges ? "bg-emerald-600 hover:bg-emerald-700 text-white font-semibold" : "border-white/10 text-white/40"} variant={hasChanges ? "default" : "outline"}>
      ...
      <div className="bg-[var(--theme-surface)] border border-white/5 rounded-lg overflow-hidden">
        <div className="p-3 border-b border-white/5"><p className="text-sm font-medium text-white">{menuItems.length} أقسام • {total} عنصر</p></div>
