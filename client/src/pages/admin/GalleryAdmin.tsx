...
const SAMPLE_IMAGES: GalleryImage[] = [
  { id: 1, title: "Pyramids at Sunset", url: "https://images.unsplash.com/photo-1539768942893-daf53e448371?w=300", category: "pyramids", tags: ["pyramids", "sunset", "giza"], featured: true, order: 1, createdAt: "2024-04-20" },
  { id: 2, title: "Nile Cruise", url: "https://images.unsplash.com/photo-1568322445389-f64ac2515020?w=300", category: "nile", tags: ["nile", "cruise", "luxury"], featured: true, order: 2, createdAt: "2024-04-19" },
  { id: 3, title: "Luxor Temple", url: "https://images.unsplash.com/photo-1553913861-c0fddf2619ee?w=300", category: "temples", tags: ["luxor", "temple", "ancient"], featured: false, order: 3, createdAt: "2024-04-18" },
  { id: 4, title: "Red Sea Diving", url: "https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=300", category: "activities", tags: ["diving", "red-sea", "coral"], featured: false, order: 4, createdAt: "2024-04-17" },
  { id: 5, title: "Desert Safari", url: "https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=300", category: "activities", tags: ["desert", "safari", "adventure"], featured: true, order: 5, createdAt: "2024-04-16" },
  { id: 6, title: "Abu Simbel", url: "https://images.unsplash.com/photo-1568322445389-f64ac2515020?w=300", category: "temples", tags: ["abu-simbel", "temple", "aswan"], featured: false, order: 6, createdAt: "2024-04-15" },
];

const CATEGORIES = ["all", "pyramids", "nile", "temples", "activities", "luxury", "food"];
...
export default function GalleryAdmin() {
  ...
  const filtered = images.filter(img => {
    const matchSearch = img.title.toLowerCase().includes(search.toLowerCase()) || img.tags.some(t => t.includes(search.toLowerCase()));
  ...
  const openCreate = () => {
    ...
    setFormData({ title: "", url: "", category: "pyramids", tags: "", featured: false });
    setIsDialogOpen(true);
  };

  const openEdit = (img: GalleryImage) => {
    setEditingImage(img);
    setFormData({ title: img.title, url: img.url, category: img.category, tags: img.tags.join(", "), featured: img.featured });
    setIsDialogOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingImage) {
      setImages(prev => prev.map(img => img.id === editingImage.id ? { ...img, ...formData, tags } : img));
    } else {
      setImages(prev => [...prev, {
        id: Date.now(),
        ...formData,
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-[var(--theme-surface)] border border-white/5 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-[var(--theme-primary)]">{images.filter(i => i.featured).length}</p>
        ...
        <div className="bg-[var(--theme-surface)] border border-white/5 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-emerald-400">{new Set(images.flatMap(i => i.tags)).size}</p>
            ...
            <div key={img.id} className="group bg-[var(--theme-surface)] border border-white/5 rounded-lg overflow-hidden hover:border-[var(--theme-primary)]/20 transition-colors">
              <div className="relative aspect-[4/3] bg-[#1a1a1a]">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  ...
                  <button onClick={() => toggleFeatured(img.id)} className="p-2 bg-white/10 rounded-lg hover:bg-white/20">
                    <Star size={16} className={img.featured ? "text-[var(--theme-primary)] fill-[var(--theme-primary)]" : "text-white"} />
                  </button>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <p className="text-[10px] text-white/30 truncate">{img.tags.join(", ")}</p>
                </div>
              </div>
            </div>
          ))}
            ...
            <tbody>
              {filtered.map(img => (
                <tr key={img.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  <td className="p-3"><img src={img.url} alt="" className="w-12 h-9 object-cover rounded" /></td>
                  <td className="p-3 text-sm text-white">{img.title}</td>
                  <td className="p-3 text-xs text-white/50">{img.category}</td>
                  <td className="p-3 text-xs text-white/30">{img.tags.join(", ")}</td>
                  <td className="p-3">
                    <button onClick={() => toggleFeatured(img.id)}>
                      <Star size={14} className={img.featured ? "text-[var(--theme-primary)] fill-[var(--theme-primary)]" : "text-white/20"} />
                    </button>
                  </td>
                  <td className="p-3">
                    <div className="flex gap-1">
          ...
          <div className="bg-[var(--theme-surface)] border border-white/10 rounded-xl w-full max-w-lg p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="bg-[#1a1a1a] border-white/10 text-white" placeholder="عنوان الصورة" />
              </div>
              <div>
                <label className="block text-sm text-white mb-1">رابط الصورة أو رفع ملف</label>
                <Input value={formData.url} onChange={(e) => setFormData({ ...formData, url: e.target.value })} className="bg-[#1a1a1a] border-white/10 text-white" placeholder="https://..." dir="ltr" />
              </div>
              <div>
                <label className="block text-sm text-white mb-1">التصنيف</label>
                <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full bg-[#1a1a1a] border border-white/10 text-white rounded-md px-3 py-2 text-sm">
              ...
              <div>
                <label className="block text-sm text-white mb-1">الوسوم (مفصولة بفواصل)</label>
                <Input value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })} className="bg-[#1a1a1a] border-white/10 text-white" placeholder="pyramids, sunset, giza" />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={formData.featured} onChange={(e) => setFormData({ ...formData, featured: e.target.checked })} className="rounded border-white/20" id="featured" />
