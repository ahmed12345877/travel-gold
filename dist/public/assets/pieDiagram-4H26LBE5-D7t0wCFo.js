...
`,"getStyles"),bt=Mt,kt=d(t=>{const a=[...t.values()].reduce((s,p)=>s+p,0),f=[...t.entries()].map(([s,p])=>({label:s,value:p})).filter(s=>s.value/a*100>=1);return mt().value(s=>s.value).sort(null)(f)},"createPieArcs"),Et=d((t,a,f,w)=>{R.debug(`rendering pie chart
`+t);const s=w.db,p=nt(),o=rt(s.getConfig(),p.pie),e=40,r=18,l=4,g=450,h=g,v=it(a),c=v.append("g");c.attr("transform","translate("+h/2+","+g/2+")");const{themeVariables:i}=p;let[x]=st(i.pieOuterStrokeWidth);x??=2;const y=o.textPosition,m=Math.min(h,g)/2-e,D=I().innerRadius(0).outerRadius(m),$=I().innerRadius(m*y).outerRadius(m*y);c.append("circle").attr("cx",0).attr("cy",0).attr("r",m+x/2).attr("class","pieOuterCircle");const u=s.getSections(),A=kt(u),C=[i.pie1,i.pie2,i.pie3,i.pie4,i.pie5,i.pie6,i.pie7,i.pie8,i.pie9,i.pie10,i.pie11,i.pie12];let M=0;u.forEach(n=>{M+=n});const L=A.filter(n=>(n.data.value/M*100).toFixed(0)!=="0"),b=gt(C).domain([...u.keys()]);c.selectAll("mySlices").data(L).enter().append("path").attr("d",D).attr("fill",n=>b(n.data.label)).attr("class","pieCircle"),c.selectAll("mySlices").data(L).enter().append("text").text(n=>(n.data.value/M*100).toFixed(0)+"%").attr("transform",n=>"translate("+$.centroid(n)+")").style("text-anchor","middle").attr("class","slice");const V=c.append("text").text(s.getDiagramTitle()).attr("x",0).attr("y",-400/2).attr("class","pieTitleText"),B=[...u.entries()].map(([n,E])=>({label:n,value:E})),k=c.selectAll(".legend").data(B).enter().append("g").attr("class","legend").attr("transform",(n,E)=>{const P=r+l,q=P*B.length/2,H=12*r,J=E*P-q;return"translate("+H+","+J+")"});k.append("rect").attr("width",r).attr("height",r).style("fill",n=>b(n.label)).style("stroke",n=>b(n.label)),k.append("text").attr("x",r+l).attr("y",r-l).text(n=>s.getShowData()?`${n.label} [${n.value}]`:n.label);const U=Math.max(...k.selectAll("text").nodes().map(n=>n?.getBoundingClientRect().width??0)),j=h+e+r+l+U,G=V.node()?.getBoundingClientRect().width??0,X=h/2-G/2,Z=h/2+G/2,N=Math.min(0,X),O=Math.max(j,Z)-N;v.attr("viewBox",`${N} 0 ${O} ${g}`),ot(v,g,O,o.useMaxWidth)},"draw"),Ft={draw:Et},Nt={parser:Tt,db:_,renderer:Ft,styles:bt};export{Nt as diagram};
